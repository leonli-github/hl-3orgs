...
createChannel() {
    ...
    if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer channel create -o orderer.example.com:7050 -c mychannel1 -f ./channel-artifacts/channel1.tx >& log1.txt
		peer channel create -o orderer.example.com:7050 -c mychannel2 -f ./channel-artifacts/channel2.tx >& log2.txt
	else
		peer channel create -o orderer.example.com:7050 -c mychannel1 -f ./channel-artifacts/channel1.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log1.txt
		peer channel create -o orderer.example.com:7050 -c mychannel2 -f ./channel-artifacts/channel2.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log2.txt
